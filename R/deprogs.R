...
dprofilerdeanalysis <- function(input = NULL, output = NULL, session = NULL, 
    ...
    iterprepDat <- reactive({
        ...
        conds <- columns[columns %in% remaining_columns]
        columns <- columns[columns %in% remaining_columns]
    ...
    observe({
        ...
        CrossScore(getFinalScores(deres(), data, columns, conds, params, 
                             ManualDEgenes = input$manualgenes, TopStat = input$topstat))
        
        # get intra condition scores and expression profiles
        IntraScore(getOutlierScores(data, columns, conds, params))
        ...
        getScoreDetails(output, session, "HomogeneityScores", CrossScore()$DEscore, CrossScore()$IterDEscore, IntraScore()$Score, IntraScore()$Score)
    ...
    list(dat = prepDat, DEgenes = deres()$DEgenes, iterdat = iterprepDat, IterDEgenes = deres()$IterDEgenes,
         CrossScore = CrossScore, IntraScore = IntraScore, deconvolute_genes = NULL, cleaned_columns = deres()$cleaned_columns)
}
               ...
               tabPanel(title = "Profiling Summary",
                        fluidRow(
                            ...
                            shinydashboard::box(title = "Membership Scores",
                                                ...
                                                column(5,actionButtonDE("gotodeconvolute", "Go to Composition Profiling", 
